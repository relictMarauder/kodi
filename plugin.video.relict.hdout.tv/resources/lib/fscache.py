...
class FSCache(object):
    ...
    def __call__(self, ignoreFirstArg):
        """
        ...
        def auto_decorator(f):
            return auto_cache_function(f, dec_self, ignoreFirstArg)
...
def make_digest(k):
    ...
    """
    s = cPickle.dumps(k, cPickle.HIGHEST_PROTOCOL)
...
def load(filename):
    ...
    # start = time.time()
    #f = bz2.BZ2File(filename, 'rb', 9)
    f = gzip.open(filename, 'rb', 9)
    # point1 = time.time()
    # f = open(filename, 'rb')
    try:
        # obj = marshal.load(f)
        ...
        f.close()
    # point2 = time.time()
    # xbmc.log( 'elapsed time: %f -%f ms \n' % ((point2-start)*1000, (point1-start)*1000))
    return obj
...
def auto_cache_function(f, cache, ignoreFirstArg):
    """
